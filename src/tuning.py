...
if __name__ == "__main__":
    ...
    # Arrotonda e salva nel file ini
    k_str = f"{best['k']:.2f}"
    beta_str = f"{best['softmax']:.2f}"

    config['Modello']['decadimento_k'] = k_str
    config['Modello']['softmax_beta'] = beta_str

    with open(config_path, 'w') as configfile:
        config.write(configfile, space_around_delimiters=False)

    print(f"\nMigliori parametri salvati in config.ini:")
    print(f"-> decadimento_k = {k_str}")
    print(f"-> softmax_beta  = {beta_str}")
